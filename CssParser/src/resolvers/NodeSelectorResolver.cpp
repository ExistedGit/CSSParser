...
namespace future {
	...
	bool NodeSelectorResolver::DoesNodeMatchSelector(const INodeSelector* node, Selector* selector)
	{
	...
	bool NodeSelectorResolver::DoesNodeMatchClassSelector(const INodeSelector* node, ClassSelector* selector)
	{
		return node->HasClass(selector->getClassIdentifier());
	}

	bool NodeSelectorResolver::DoesNodeMatchIDSelector(const INodeSelector* node, IdSelector* selector)
	{
		return node->GetId() == selector->getIdIdentifier();
	}

	bool NodeSelectorResolver::DoesNodeMatchTypeSelector(const INodeSelector* node, TypeSelector* selector)
	{
	...
	bool NodeSelectorResolver::DoesNodeMatchAttributeSelector(const INodeSelector* node, AttributeSelector* selector)
	{
		...
		auto oAttr = node->GetAttribute(key);
		if (!oAttr) return false;

		const auto& attr = *oAttr;

		std::string nodeAttrValue = StringUtil::tolower(attr);
	...
	bool NodeSelectorResolver::DoesNodeMatchPseudoSelector(const INodeSelector* node, PseudoSelector* selector)
	{
		return NodePseudoSelectorResolver::DoesNodeMatchPseudo(node, selector);
	}

	bool NodeSelectorResolver::DoesNodeMatchSequenceSelector(const INodeSelector* node, SequenceSelector* selector)
	{
	...
	bool NodeSelectorResolver::DoesNodeMatchGroupSelector(const INodeSelector* node, GroupSelector* selector)
	{
	...
	bool NodeSelectorResolver::DoesNodeMatchCombineSelector(const INodeSelector* node, CombineSelector* selector)
	{
		...
		std::vector<const INodeSelector*> nodesToCheck;
		...
		for (size_t i = 0; i < nodesToCheck.size(); i++) {
			...
			case CombineSelector::InstanceSibling: {
				auto children = parent->GetChildren();
				const INodeSelector* lastElementNode = nullptr;
				for (const auto& sibling : children) {
